...
for FILE_LOCATION in $(echo ${FILE_LOCATIONS}); do
    ...
    for IMAGE_POLICY in "${IMAGE_POLICIES[@]}"; do
        ...
        DIRECTORIES=$(find apps -type d -not -path '*dev*' -not -path '*stg*' -not -path '*test*' -not -path '*ithc*' -not -path '*demo*' -not -path '*sbox*' -not -path 'apps' -not -path '*toffee*')

        for dir in $DIRECTORIES; do

            ./kustomize build --load-restrictor LoadRestrictionsNone "$dir" 2>&1 >> $OUTPUTFILE  | yq eval 'select(.kind == "HelmRelease" and (.spec.values.nodejs.image != null or .spec.values.java.image != null))' >> $OUTPUTFILE
