...
for FILE_LOCATION in $(echo ${FILE_LOCATIONS}); do
    ...
    for IMAGE_POLICY in "${IMAGE_POLICIES[@]}"; do
        ...
        for dir in $DIRECTORIES; do
            ...
            ./kustomize build --load-restrictor LoadRestrictionsNone "$dir" 2>&1 | yq eval 'select(.kind == "HelmRelease" and (.spec.values.nodejs.image != null or .spec.values.java.image != null))' >> images.yaml
