...
for FILE_LOCATION in $(echo ${FILE_LOCATIONS}); do
    ...
    for IMAGE_POLICY in "${IMAGE_POLICIES[@]}"; do
        ...
        IMAGE_POLICY_NAME="${IMAGE_POLICY}"
        IMAGE_AUTOMATION_CHECK=$(cat imagepolicies_list.yaml  | \
        yq eval 'select(.metadata and .kind == "ImagePolicy" and .metadata.name == env(IMAGE_POLICY_NAME) )' - | yq eval '.spec.filterTags.pattern == "^prod-[a-f0-9]+-(?P<ts>[0-9]+)"' -)
