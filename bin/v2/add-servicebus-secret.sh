...
NAMESPACE="$1"
ENVIRONMENT="${2:-dev}"
SUBSCRIPTION=${3:-DTS-SHAREDSERVICES-STG}
SOPS_KEY=${4:-https://dtssharedservicesdevkv.vault.azure.net/keys/sops-key/2beba064ddfe454482ad0133af2cf0fd}
...
elif [ $(SECRET_FILE_NAME="${SECRET_FILE_NAME}"  yq 'contains({"resources": [env(SECRET_FILE_NAME)]})' apps/${NAMESPACE}/${ENVIRONMENT}/sops-secrets/kustomization.yaml) == false ]; then
