...
for file in $(grep -lr "kind: HelmRelease" $FILE_DIRECTORY); do
  ...
  if $(yq e '.spec | has("chart")' $file) ; then
    ...
    if $(yq e '.spec.chart | has("repository")' $file) ; then
      ...
      if [[ $CHART_GIT == *"hmcts/sds-flux-config"* ]]; then
       yq e '.spec.chart.spec.sourceRef.kind = "GitRepository"|.spec.chart.spec.sourceRef.name = "flux-config"| .spec.chart.spec.sourceRef.namespace = "flux-system" | .spec.chart.spec.interval = "1m"' -i $file
      fi
    fi
  fi
done